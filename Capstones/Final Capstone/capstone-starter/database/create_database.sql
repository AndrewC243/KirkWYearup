...
-- homebrew stored procedures
DELIMITER //
CREATE PROCEDURE ConvertSCI(IN orderid INT, IN userid INT, IN discount DECIMAL(10,2))
BEGIN
	INSERT INTO order_line_items (order_id,product_id,sales_price,quantity,discount)
    SELECT orderid,shopping_cart.product_id,price*quantity*(1-discount),quantity,discount
    FROM shopping_cart
...
CREATE PROCEDURE CreateOrder(userid INT, shipping_amount DECIMAL(10,2))
BEGIN
	DECLARE rightnow DATETIME DEFAULT current_timestamp();
    DECLARE orderid INT;
    
    INSERT INTO orders (user_id,date,address,city,state,zip,shipping_amount)
    SELECT userid,rightnow,address,city,state,zip,shipping_amount
    FROM profiles
